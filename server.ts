...
app.post("/api/translate", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "translation_failed", details: err?.response?.data ?? err.message });
  }
});

app.listen(PORT, () => console.log(`Translator proxy running on http://localhost:${PORT}`));
